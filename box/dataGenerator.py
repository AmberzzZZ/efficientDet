...
def data_generator(data_dir, batch_size, input_shape, num_classes, anchors, strides=[8,16,32],
    ...
    while 1:
        ...
        image_batch = np.zeros((batch_size, input_shape[0], input_shape[1], 3))
        ...
        for i in range(batch_size):
            ...
            img = cv2.imread(os.path.join(data_dir, file_name+'.png'), 1)
            if np.max(img)>1:
                img = img / 255.
        ...
        for idx, s in enumerate(strides):
            ...
            if len(coords[0]):
                ...
                for obj in range(len(coords[0])):
                    ...
                    # print("box: ", box)
                    xc, yc, w, h = box[:4]
                    cv2.rectangle(img, (int(640*(xc-w/2.)), int(480*(yc-h/2.))), (int(640*(xc+w/2.)), int(480*(yc+h/2.))), (255,0,0), 2)
